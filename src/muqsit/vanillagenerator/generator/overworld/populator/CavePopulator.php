...
class CavePopulator implements Populator
{
	...
	public function populate(ChunkManager $world, Random $random, int $chunk_x, int $chunk_z, Chunk $chunk): void
	{
		...
		for ($currentChunkX = $chunk_x - self::CAVE_RANGE; $currentChunkX <= $chunk_x + self::CAVE_RANGE; $currentChunkX++) {
			for ($currentChunkZ = $chunk_z - self::CAVE_RANGE; $currentChunkZ <= $chunk_z + self::CAVE_RANGE; $currentChunkZ++) {
				$rx = (int)($currentChunkX * $j);
				$rz = (int)($currentChunkZ * $k);

				$this->random->setSeed($rx ^ $rz ^ $random->getSeed());

				$this->recursiveGenerate($currentChunkX, $currentChunkZ, $chunk_x, $chunk_z, $chunk, true, $allCondition);
			}
		}
	}

	public static function signLong(int $value) : int {
		return $value << 64 >> 64;
	}

	public static function unsignLong(int $value) : int {
	...
	private function digBlock(Chunk $chunk, int $currX, int $currY, int $currZ, int $caveLiquidAltitude): void
	{
		if (self::canReplaceBlock($chunk, new Vector3($currX, $currY, $currZ))) {
			if ($currY - 1 < $caveLiquidAltitude) {
				$chunk->setBlockStateId($currX, $currY, $currZ, VanillaBlocks::LAVA()->getStateId());
			} else {
				$chunk->setBlockStateId($currX, $currY, $currZ, VanillaBlocks::AIR()->getStateId());

				// Try to repair naked dirt by replacing them with grass if they are the highest block here
				$block = RuntimeBlockStateRegistry::getInstance()->fromStateId($chunk->getBlockStateId($currX, $currY - 1, $currZ));
				if ($block->hasTypeTag(BlockTypeTags::DIRT) && $chunk->getHighestBlockAt($currX, $currZ) === ($currY - 1)) {
					$chunk->setBlockStateId($currX, $currY - 1, $currZ, VanillaBlocks::GRASS()->getStateId());
				}
			}
		}
	}
	...
	public static function canReplaceBlock(Chunk $chunk, Vector3 $blockPos): bool
	{
		...
		$block = $runtimeBlockStateRegistry->fromStateId($chunk->getBlockStateId($curX, $curY, $curZ));
		$blockAbove = $runtimeBlockStateRegistry->fromStateId($chunk->getBlockStateId($curX, $curY + 1, $curZ));

		// Avoid damaging trees
		if ($block instanceof Leaves || in_array($block->getTypeId(), [
		...
		if (in_array($blockAbove->getTypeId(), [
			BlockTypeIds::ACACIA_LOG,
			BlockTypeIds::BIRCH_LOG,
			BlockTypeIds::DARK_OAK_LOG,
			BlockTypeIds::JUNGLE_LOG,
		...
		if ($curY > self::CAVE_LIQUID_ALTITUDE) {
			foreach (Facing::ALL as $facing) {
				...
				if (($facingSide->getX() >= 0 && $facingSide->getX() <= 15) && ($facingSide->getZ() >= 0 && $facingSide->getZ() <= 15)) {
					...
					$blockArea = $runtimeBlockStateRegistry->fromStateId($chunk->getBlockStateId($facingSide->getX(), $facingSide->getY(), $facingSide->getZ()));
		...
		// Mine-able blocks
		if ($block->hasTypeTag(BlockTypeTags::DIRT) || in_array($block->getTypeId(), [
			BlockTypeIds::STONE,
		...
		return ($block->hasTypeTag(BlockTypeTags::SAND)|| $block->getTypeId() === BlockTypeIds::GRAVEL)
