...
class Lake extends TerrainObject{
	...
	public function generate(ChunkManager $world, Random $random, int $source_x, int $source_y, int $source_z) : bool{
		...
		$biome = $chunk->getBiomeId(($source_x + 8 + (int) self::MAX_DIAMETER / 2) & Chunk::COORD_MASK,7, ($source_z + 8 + (int) self::MAX_DIAMETER / 2) & Chunk::COORD_MASK);
			...
			for($z = 0; $z < $max_diameter; ++$z){
				for($y = 0; $y < $max_diameter; ++$y){
					...
					if($y >= (int) (self::MAX_HEIGHT / 2)){
						...
						if($type instanceof Water && $type->isStill() && BiomeClimateManager::isCold($chunk->getBiomeId($x & Chunk::COORD_MASK, 7,$z & Chunk::COORD_MASK), $source_x + $x, $y, $source_z + $z)){
